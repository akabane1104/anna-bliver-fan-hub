...
CREATE TABLE user_bilibili_bindings (
  ...
  target_anchor_uid BIGINT UNSIGNED DEFAULT NULL,
  target_room_id BIGINT UNSIGNED DEFAULT NULL,
  fans_medal_level INT UNSIGNED DEFAULT NULL,
  fans_medal_name VARCHAR(100) DEFAULT NULL,
  fans_medal_status ENUM('unknown','active','inactive') NOT NULL DEFAULT 'unknown',
  guard_level TINYINT UNSIGNED DEFAULT NULL,
  guard_started_at DATETIME(3) DEFAULT NULL,
  guard_expires_at DATETIME(3) DEFAULT NULL,
  identity_sync_status ENUM('never','pending','success','failed','unavailable') NOT NULL DEFAULT 'never',
  identity_source ENUM('transient_qr','server_provider','official_listener','manual_fallback') DEFAULT NULL,
  last_sync_attempt_at DATETIME(3) DEFAULT NULL,
  last_sync_success_at DATETIME(3) DEFAULT NULL,
  last_sync_error_code VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin DEFAULT NULL,
  identity_observed_at DATETIME(3) DEFAULT NULL,
  identity_version BIGINT UNSIGNED NOT NULL DEFAULT 0,
  sync_failure_count INT UNSIGNED NOT NULL DEFAULT 0,
  next_sync_at DATETIME(3) DEFAULT NULL,
  manual_role ENUM('fan_club','captain','admiral','governor') DEFAULT NULL,
  manual_expires_at DATETIME(3) DEFAULT NULL,
  manual_actor_user_id INT DEFAULT NULL,
  manual_reason VARCHAR(500) DEFAULT NULL,
  manual_created_at DATETIME(3) DEFAULT NULL,
  manual_overridden_at DATETIME(3) DEFAULT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  ...
  INDEX idx_binding_identity_due (identity_sync_status, next_sync_at),
  INDEX idx_binding_guard_expiry (guard_expires_at),
  INDEX idx_binding_manual_expiry (manual_expires_at),
  CONSTRAINT fk_binding_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  CONSTRAINT fk_binding_manual_actor FOREIGN KEY (manual_actor_user_id) REFERENCES users(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS viewer_identity_audit (
  ...
  CONSTRAINT fk_viewer_identity_binding FOREIGN KEY (binding_id) REFERENCES user_bilibili_bindings(id) ON DELETE SET NULL,
  CONSTRAINT fk_viewer_identity_target FOREIGN KEY (target_user_id) REFERENCES users(id),
  CONSTRAINT fk_viewer_identity_actor FOREIGN KEY (actor_user_id) REFERENCES users(id) ON DELETE SET NULL
